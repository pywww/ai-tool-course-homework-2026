...
你自己的 Gitee 仓库  <────────────────┐
      │                               │
      ↓ 2. Clone (下载到本地)         │ 6. Push (推送到云端)
      │                               │
你电脑上的本地仓库  ──────────────────┘
      │
      ↓ 7. PR (Pull Request，网页端操作)
      │
老师的 Gitee 仓库 (等待老师 Review 批改)
```
...
2. **确保你的 master 分支是最新的**：
   *(如果你刚刚克隆，这一步可以跳过。如果是之后再次做作业，请先在Gitee网页点击"同步"，然后再执行以下命令)*
   ...
   ```
3. **创建并切换到你的个人分支**（**强烈建议使用拼音或英文命名，避免中文编码问题**）：
   ```bash
   git checkout -b homework/拼音姓名_你的学号
   ```

   **示例**：
   ```bash
   git checkout -b homework/zhangsan_2023001
   ```
...
**✅ 成功标准**：
- 命令行显示当前分支为 `homework/zhangsan_2023001`（有 `*` 标记）
- 你现在可以安全地修改文件了！
...
**示例**：
```bash
git add exercises/student_info_zhangsan_2023001.txt
```
...
```bash
# 从上游（老师的仓库）的 master 分支拉取最新内容，并合并到当前分支
git pull upstream master
*(注意：这比先切换到本地 master 再 pull 更安全，可以避免新手不小心污染本地 master 分支)*
...
3. **推送到你自己的Gitee仓库**：
   ```bash
   git push origin homework/拼音姓名_你的学号
   ```

   **示例**：
   ```bash
   git push origin homework/zhangsan_2023001
   ```
...
3. **创建新的作业分支**：
   ```bash
   git checkout -b homework/拼音姓名_作业2
   ```
...
2. **每次提交前先同步**：`git pull upstream master` 是你的好朋友
