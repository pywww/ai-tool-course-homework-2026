...
**步骤2：填写注册信息**
- 输入你的手机号或邮箱地址
- 创建一个用户名（建议使用真实姓名或易于识别的名称，如：zhang_san_2025）
- 设置一个强密码
- 完成验证（输入验证码）
...
2. **每次提交前先同步**：`git pull origin master` 是你的好朋友
...
```mermaid
flowchart TD
    A[课前准备<br/>注册Gitee + 安装Git] --> B[克隆仓库到本地]
    B --> C[创建个人分支<br/>homework/姓名_学号]
    C --> D[完成作业]
    D --> E{准备提交?}
    E -->|是| F[切换到master<br/>git pull]
    F --> G[切换回个人分支<br/>git merge master]
    G --> H{有冲突?}
    H -->|是| I[手动解决冲突]
    I --> J[git add + git commit]
    H -->|否| K[git push 到个人分支]
    J --> K
    K --> L[在Gitee创建Pull Request]
    L --> M[等待老师审核]
```
